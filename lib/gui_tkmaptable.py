...
class MapTable(tk.Canvas):
	...
	def __init__(self, parent, xname, read_xdata, yname, read_ydata, name, read_data, write_cell=lambda x,y,value:None, xfmt="{:d}", yfmt="{:d}", datafmt="{:d}"):
		...
		self.create_text(self.CELLW-2, 3, anchor=tk.NE, justify=tk.RIGHT, text=xname, font=('Helvetica 6'))
		self.create_text(2, self.CELLH-3, anchor=tk.NW, text=yname,  font=('Helvetica 6'))
		self.xaxis = [self.create_text(self.CELLW*(x+1)+(self.CELLW/2), 6, anchor=tk.N, justify=tk.CENTER, text=xfmt.format(self.xdata[x]), font=('Helvetica 8')) for x in range(0,self.xsize)]
		self.yaxis = [self.create_text(self.CELLH, self.CELLH*(y+1)+6, anchor=tk.N, justify=tk.CENTER, text=yfmt.format(self.ydata[y]), font=('Helvetica 8')) for y in range(0,self.ysize)]
		...
		for y in range(0,self.ysize):
			for x in range(0,self.xsize):
				...
				self.cells[y][x][1] = self.create_text(px+self.CELLW-3, py+3, anchor=tk.NE, justify=tk.RIGHT, text=datafmt.format(self.data[y][x]), font=('Helvetica 8 bold'))
	...
	def draw_cursor(self):
		...
		px=self.CELLW*(self.interpolation[0]+1)
		py=self.CELLH*(self.interpolation[1]+1)
		px2=px+self.CELLW*(self.interpolation[2]+0.5)
		py2=py+self.CELLH*(self.interpolation[3]+0.5)
		self.cursor = (
			self.create_rectangle(px, py, px+(self.CELLW*2), py+(self.CELLH*2), outline='red', width=4),
			self.create_line(px2, self.CELLH, px2, py, fill="red", width=2),
			self.create_line(self.CELLW*2, py2, px, py2, fill="red", width=2),
			self.create_line(px2, py+(self.CELLH*2), px2, (self.ysize+1)*self.CELLH, fill="red", width=2),
			self.create_line(px+(self.CELLW*2), py2, (self.xsize+1)*(self.CELLW*2), py2, fill="red", width=2)
		)
	...
	def do_interpolation(self, xvalue, yvalue):
		# Find X and Y cell
		cx = cy = 0
		while(self.xdata[cx] == self.xdata[cx+1]): cx = cx + 1;
		for x in range(cx,self.xsize-1):
			if(xvalue >= self.xdata[x]): cx = x
			else: break
		while(self.ydata[cy] == self.xdata[cy+1]): cy = cy + 1;
		for y in range(cy,self.ysize-1):
		...
		x2r = (diff/step if step > 0 else 1.0) if diff > 0 else 0.0
		step = self.ydata[cy+1]-self.ydata[cy]
		diff = yvalue-self.ydata[cy]
		y2r = (diff/step if step > 0 else 1.0) if diff > 0 else 0.0
		x1r = 1.0 - x2r
		y1r = 1.0 - y2r
	...
	def on_motion(self, event):
		if(self.selection[5]):
			...
			self.selection[3] = sorted([self.selection[1]+1, (event.y//self.CELLH), self.xsize])[1]
			self.draw_selection()
...
class MapTableEditor(tk.Frame):
	def __init__(self, parent, xname, read_xdata, yname, read_ydata, name, read_data, write_cell=lambda x,y,value:None, xfmt="{:d}", yfmt="{:d}", datafmt="{:d}", step=1.0):
	...
	def update(self, xvalue, yvalue):
		self.table.do_interpolation(xvalue, yvalue)
...
class TextGauge(tk.LabelFrame):
	def __init__(self, parent, description, fmt="{:d}", font=('Arial', 25)):
		tk.Frame.__init__(self, parent)
		tk.Label(self, text=description).pack()
		self.string = tk.StringVar()
		tk.Entry(self, width=16, state="readonly", justify="right", textvariable=self.string).pack()
		self.fmt = fmt
		self.update(0)
		self.pack()
	def update(self, value):
		self.string.set(self.fmt.format(value))
