...
class CRP08_uploader:
	...
	def open_can(self, chunk_can):
		...
		self.p.log(f"Open CAN {self.interface} {self.channel} @ {chunk_can.can_bitrate} kbit/s")
		self.bus = can.Bus(
	...
	def bootstrap(self, crp, timeout=60, ui_cb=lambda:None):
		...
		self.p.log(f"Power On ECU, please! (within {timeout} seconds)")
		while(True):
			...
			if(cmd == 0x04 or cmd == 0x05):
				error = data[4]
				raise CRP08_exception(f"ECU: Error {hex(error)} " + self.errors.get(error, "Unknown"))
		self.close_can()
