...
class ECU_T4E:
	...
	def readMemory(self, address, size):
		...
		try:
			if  (size == 4):
				...
				id, dlc, data = struck.unpack(CAN_HDR_FRMT+"4s4x", cf)
				...
				self.sock.send(cf)
				cf = self.sock.recv(CAN_FRA_SIZE)
				id, dlc, data = struck.unpack(CAN_HDR_FRMT+"2s6x", cf)
				...
				self.sock.send(cf)
				cf = self.sock.recv(CAN_FRA_SIZE)
				id, dlc, data = struck.unpack(CAN_HDR_FRMT+"1s7x", cf)
				...
				while(size > 0):
					...
					cf = self.sock.recv(CAN_FRA_SIZE)
					id, dlc, chunk = struck.unpack(CAN_HDR_FRMT+"%ds%dx" \
	...
	def inject(self, freeram_address, filename, stackblr_address):
		...
		#ba_opcode = b'\x48\x00\x00\x02'
		#ba_opcode[1] |= (jump_addr >> 16) & 0xFF
		#ba_opcode[2] |= (jump_addr >>  8) & 0xFF
		#ba_opcode[3] |= jump_addr & 0xFC
		#self.writeMemory(0x3F8000+0x7FDC, ba_opcode, False)
		
	def test(self, freeram_address):
		# Word
		self.writeMemory(stackblr_address, b'\xDE\xAD\xBE\xEF')
		# 3 Bytes
		self.writeMemory(stackblr_address, b'\x11\x22\x33')
		# Half
		self.writeMemory(stackblr_address, b'\xAA\x55')
		# Byte
		self.writeMemory(stackblr_address, b'\x10')
		# Much more
		self.writeMemory(stackblr_address, b'Hello world')
...
if __name__ == "__main__":
	...
	if(ecu_op == 'ifp'):
		...
		t4e.upload(0x3FE748, "injection/deadloop.bin", 0x3F8000 + 0x7FDC)
