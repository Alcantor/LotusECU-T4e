...
public class MPC561_RegisterMap extends GhidraScript {
	...
	public void run() throws Exception {
		...
		// Create BBC registers (Only for MPC562/MPC564)
		//createBBCRegisters();
		...
		// Create CALRAM registers (Unused)
	...
	private void createTPU3Registers() throws Exception {
		...
		for (int ch = 0; ch < 16; ch++) {
			for (int p = 0; p < 8; p++) {
				long baseAddr = 0x00304100L + (ch * 0x10) + (p * 2);
				createRegLabel(baseAddr, "TPU3A_CH" + ch + "_PARAM" + p, 2);
			}
		}
		...
		for (int ch = 0; ch < 16; ch++) {
			for (int p = 0; p < 8; p++) {
				long baseAddr = 0x00304500L + (ch * 0x10) + (p * 2);
				createRegLabel(baseAddr, "TPU3B_CH" + ch + "_PARAM" + p, 2);
			}
		}
	}
	...
	private void createQADCRegisters() throws Exception {
		...
		createRegLabel(0x00304806L, "PORTQA_A", 1);
		createRegLabel(0x00304807L, "PORTQB_A", 1);
		createRegLabel(0x00304808L, "DDRQA_A", 1);
		createRegLabel(0x00304809L, "DDRQB_A", 1);
		...
		createRegLabel(0x00304C06L, "PORTQA_B", 1);
		createRegLabel(0x00304C07L, "PORTQB_B", 1);
		createRegLabel(0x00304C08L, "DDRQA_B", 1);
		createRegLabel(0x00304C09L, "DDRQB_B", 1);
	...
	private void createQSMCMRegisters() throws Exception {
		...
		createRegLabel(0x00305016L, "PQSPAR", 1);
		createRegLabel(0x00305017L, "DDRQST", 1);
	...
	private void createTouCANModule(String suffix, long ctrlBase, long mbuffBase) throws Exception {
		...
		for (int buf = 0; buf <= 15; buf++) {
			...
			createRegLabel(bufBase + 0x06, "CAN" + suffix + "_MB" + buf + "_DATA0", 1);
			createRegLabel(bufBase + 0x07, "CAN" + suffix + "_MB" + buf + "_DATA1", 1);
			createRegLabel(bufBase + 0x08, "CAN" + suffix + "_MB" + buf + "_DATA2", 1);
			createRegLabel(bufBase + 0x09, "CAN" + suffix + "_MB" + buf + "_DATA3", 1);
			createRegLabel(bufBase + 0x0A, "CAN" + suffix + "_MB" + buf + "_DATA4", 1);
			createRegLabel(bufBase + 0x0B, "CAN" + suffix + "_MB" + buf + "_DATA5", 1);
			createRegLabel(bufBase + 0x0C, "CAN" + suffix + "_MB" + buf + "_DATA6", 1);
			createRegLabel(bufBase + 0x0D, "CAN" + suffix + "_MB" + buf + "_DATA7", 1);
		}
	}
